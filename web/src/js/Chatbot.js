...
class Chatbot {
  ...
  // Supabase 소멸(2026-07) — 동일 오리진 백엔드 shim(/rest/v1)으로 전환
  static SUPABASE_URL = '';
...
14. When two Korean names are mentioned, ALWAYS include BOTH in the WHERE clause with OR.`;
  ...
  constructor() {
    ...
    this._korNameMap = {
      '여찬혁': 'YEO Chanhyuk', '김지수': 'KIM Jisoo', '홍수정': 'HONG Sujung',
      '정승기': 'JUNG Seunggi', '신연수': 'SHIN Yeonsu', '이승훈': 'LEE Seunghun',
      '정예은': 'JUNG Yeeun', '안재웅': 'AHN Jaeung', '박예운': 'PARK Yewoon',
      '송영민': 'SONG Youngmin', '곽은우': 'KWACK Eunwoo',
    };
...
6. If no function matches, reply "general_sql".` },
  ...
  async _pipeline(question) {
    ...
    // Phase 0~Shortcuts removed: all questions go through dynamic pipeline
    const korNames = this._extractKoreanNames(question);
    ...
    const finalSQL = this._injectFilters(rawSQL, tables, question);
      ...
      if (_resolved.length >= 2) {
        ...
        try {
          ...
          if (Array.isArray(_data) && _data.length > 0) {
            ...
            if (aggregated.per_player && Object.keys(aggregated.per_player).length >= 2) {
              const NL = String.fromCharCode(10);
              let md = '## ' + _compareKorNames.join(' vs ') + ' 기록 비교' + NL + NL;
              md += '| 선수 | 경기 수 | 평균 | 최고 | 최저 |' + NL;
              md += '|------|---------|------|------|------|' + NL;
              for (const [pname, ps] of Object.entries(aggregated.per_player)) {
                md += '| ' + pname + ' | ' + ps.count + '건 | ' + ps.finish_avg + '초 | ' + ps.finish_min + '초 | ' + ps.finish_max + '초 |' + NL;
              }
              const entries = Object.entries(aggregated.per_player);
              if (entries.length >= 2) {
                const diff = (entries[1][1].finish_min - entries[0][1].finish_min).toFixed(2);
                const faster = parseFloat(diff) > 0 ? entries[0][0] : entries[1][0];
                md += NL + '> **' + faster + '**이(가) 최고 기록 기준 ' + Math.abs(parseFloat(diff)) + '초 빠름';
              }
              // Wrap with LLM for natural tone
              try {
                const wrapResp = await this._callLLM([
                  { role: 'system', content: 'You are a sports coach. Rewrite the following data comparison in warm Korean. KEEP ALL numbers exactly. Use markdown. Add brief analysis.' },
                  { role: 'user', content: md },
                ], 0.3);
                return { text: wrapResp };
              } catch(e) {
                return { text: md };
              }
            }
          // Build explicit compare context for LLM
          let _compareCtx = '';
          if (aggregated.per_player) {
            for (const [pname, pstats] of Object.entries(aggregated.per_player)) {
              _compareCtx += pname + ': ' + pstats.count + '건, 평균 ' + pstats.finish_avg + '초, 최고 ' + pstats.finish_min + '초, 최저 ' + pstats.finish_max + '초' + String.fromCharCode(10);
            }
          }
            const answers = await Promise.all(
              [0, 0.3, 0.6, 0.9].map(t => this._generateAnswer(question, _data, 'REST or= direct', aggregated, t))
            );
            let bestIdx = 0;
            try {
              const NL = String.fromCharCode(10);
              const labels = ['A','B','C','D'];
              const jp = answers.map((a,i) => '[Answer ' + labels[i] + ']' + NL + a).join(NL+NL+'---'+NL+NL);
              const jr = await this._callLLM([
                { role: 'system', content: 'You are a strict answer judge. Evaluate on 5 criteria (1-10): 1)Relevance 2)Accuracy 3)Comparison 4)Formatting 5)Insight. Reply ONLY JSON: {"best":"X"}' },
                { role: 'user', content: 'Question: ' + question + NL + 'DB Stats: ' + JSON.stringify(aggregated) + NL + NL + jp },
              ], 0);
              const bm = jr.match(/"best"\s*:\s*"([ABCD])"/);
              if (bm) bestIdx = 'ABCD'.indexOf(bm[1]);
            } catch(e) { console.log("[CMP-ERR]", e.message || e); }
            let finalAnswer = this._codeFactcheck(answers[bestIdx], _data, aggregated);
            if (!finalAnswer || finalAnswer.length < 20) finalAnswer = this._templateFallback(question, _data, aggregated);
  ...
  _clientAggregate(data) {
    // Per-player aggregation for compare questions
    const byPlayer = {};
    for (const r of data) {
    ...
    if (Object.keys(byPlayer).length >= 2) {
      const playerStats = {};
      for (const [name, rows] of Object.entries(byPlayer)) {
        const finishes = rows.map(r => parseFloat(r.finish)).filter(v => !isNaN(v) && v > 0);
        if (finishes.length > 0) {
          playerStats[name] = {
            count: finishes.length,
            finish_avg: +(finishes.reduce((a,b) => a+b, 0) / finishes.length).toFixed(3),
            finish_min: +Math.min(...finishes).toFixed(3),
            finish_max: +Math.max(...finishes).toFixed(3),
          };
        }
      }
...
Generate a single SELECT SQL query:`;
  ...
  _injectFilters(sql, tables, question) {

    const upper = sql.toUpperCase();

    // Force inject player names: remove existing name filter, add OR for all mentioned players
    if (question) {
      const korNames = this._extractKoreanNames(question);
      if (korNames.length >= 2) {
        const resolved = korNames.map(n => this._korNameMap[n]).filter(Boolean);
        if (resolved.length >= 2) {
          // Remove existing name ILIKE conditions
          sql = sql.replace(/AND\s+name\s+ILIKE\s+'[^']*'/gi, '');
          sql = sql.replace(/name\s+ILIKE\s+'[^']*'\s+AND/gi, '');
          sql = sql.replace(/name\s+ILIKE\s+'[^']*'/gi, '');
          sql = sql.replace(/name\s*=\s*'[^']*'/gi, '');
          // Build OR condition for all players
          const nameConditions = resolved.map(eng => "name ILIKE '" + eng.split(' ')[0] + "%'").join(' OR ');
          if (/WHERE/i.test(sql)) {
            sql = sql.replace(/WHERE\s+/i, 'WHERE (' + nameConditions + ') AND ');
          } else {
            const insertPt = sql.search(/\s+(ORDER|LIMIT|GROUP)\s+/i);
            if (insertPt > 0) sql = sql.slice(0, insertPt) + ' WHERE (' + nameConditions + ')' + sql.slice(insertPt);
            else sql = sql + ' WHERE (' + nameConditions + ')';
          }
          // Ensure LIMIT is at least 200
...
6. CRITICAL for COMPARE questions (vs, 비교): You MUST analyze EACH player separately with their own stats (avg, min, max, count). Group the data by 'name' column and present BOTH players in a markdown table.
7. If data is insufficient, say "데이터가 부족합니다".` },

      { role: 'user', content: 'Question: ' + question + String.fromCharCode(10) + 'DB Result (' + totalRows + ' rows, sample ' + sampleData.length + '):' + String.fromCharCode(10) + dataStr + String.fromCharCode(10) + 'AGGREGATED:' + String.fromCharCode(10) + aggStr + (aggregated && aggregated.per_player ? String.fromCharCode(10) + 'PER-PLAYER:' + String.fromCharCode(10) + JSON.stringify(aggregated.per_player) : '') + String.fromCharCode(10) + 'Answer:' },
